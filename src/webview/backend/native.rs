...
impl NativeBackend {
    ...
    fn create_webview(
        ...
        // Conditionally attach the drag-drop proxy handler (RFC 0015).
        //
        // - `capture_file_drop = false` (default): the WebView falls back
        //   to browser-native HTML5 dragenter/dragover/drop.
        // - `capture_file_drop = true`: dispatch via IPC `file_drop_*`
        //   events through the IpcHandler sink. Note that enabling this
        //   suppresses HTML5 drag-drop inside the WebView (upstream
        //   wry/WebView2 limitation, see RFC 0015 §2).
