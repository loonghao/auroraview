...
impl OverlayWriter {
    ...
    ///
    /// Uses `append_data` instead of `append` to properly handle long paths
    /// (paths > 100 characters) using GNU tar extensions.
    fn create_assets_archive(assets: &[(String, Vec<u8>)]) -> PackResult<Vec<u8>> {
        ...
        for (path, content) in assets {
            ...
            header.set_mtime(0); // Use epoch time for reproducibility
            header.set_cksum();

            // Use append_data which handles long paths automatically via GNU extensions
