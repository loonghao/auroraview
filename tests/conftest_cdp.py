...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "cdp: mark test as requiring CDP connection"
    )
    config.addinivalue_line(
        "markers", "requires_gallery: mark test as requiring gallery process"
    )
...
class GalleryProcessManager:
    ...
    def __init__(self, exe_path: Optional[Path] = None):
        self.exe_path = exe_path or (PROJECT_ROOT / "gallery" / "pack-output" / "auroraview-gallery.exe")
