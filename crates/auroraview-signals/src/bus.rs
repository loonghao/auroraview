...
impl EventBus {
    ...
    pub fn emit_to(&self, bridge_name: &str, event: &str, data: Value) -> Result<(), BridgeError> {
        ...
        self.bridges.emit_to_named(bridge_name, event, data)
    }
...
mod tests {
    ...
    #[test]
    fn test_emit_to_targets_specific_bridge() {
        let bus = EventBus::new();
        ...
        bus.add_bridge(CallbackBridge::new("bridge_a", move |_, _| {
            ca.fetch_add(1, Ordering::SeqCst);
            Ok(())
        }));
        bus.add_bridge(CallbackBridge::new("bridge_b", move |_, _| {
            cb.fetch_add(1, Ordering::SeqCst);
            Ok(())
        }));

        // emit_to should only reach bridge_a, not bridge_b
        bus.emit_to("bridge_a", "test:event", json!(null)).unwrap();
    ...
    #[test]
    fn test_emit_to_nonexistent_bridge() {
        let bus = EventBus::new();
        bus.add_bridge(CallbackBridge::new("bridge_a", |_, _| Ok(())));

        let result = bus.emit_to("nonexistent", "test:event", json!(null));
