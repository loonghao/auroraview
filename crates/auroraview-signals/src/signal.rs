...
impl<T: Clone + Send + 'static> Signal<T> {
    ...
    pub fn emit(&self, value: T) {
        // Clone handlers before calling them to avoid holding the read lock
        // during handler execution. This prevents deadlocks when a handler
        // calls connect/disconnect on the same signal.
        let handlers: Vec<Handler<T>> = {
            let guard = self.handlers.read();
            tracing::trace!(
                signal_name = ?self.name,
                handler_count = guard.len(),
                "Emitting signal"
            );
