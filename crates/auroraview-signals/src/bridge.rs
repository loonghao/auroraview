...
impl MultiBridge {
    ...
    ///
    /// Unlike `emit()` which broadcasts to all bridges, this method
    /// targets a single bridge identified by name.
    pub fn emit_to_named(
        &self,
        bridge_name: &str,
        event: &str,
        data: Value,
        ...
        for bridge in bridges.iter() {
            if bridge.name() == bridge_name {
                if !bridge.is_connected() {
                    return Err(BridgeError::Disconnected(bridge_name.to_string()));
                }
                return bridge.emit(event, data);
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_emit_to_named_targets_specific_bridge() {
        let count1 = Arc::new(AtomicUsize::new(0));
        let count2 = Arc::new(AtomicUsize::new(0));
        ...
        let multi = MultiBridge::new("multi");
        multi.add(CallbackBridge::new("bridge_a", move |_, _| {
            c1.fetch_add(1, Ordering::SeqCst);
            Ok(())
        }));
        multi.add(CallbackBridge::new("bridge_b", move |_, _| {
            c2.fetch_add(1, Ordering::SeqCst);
            Ok(())
        }));

        // emit_to_named should only reach bridge_a
        multi
            .emit_to_named("bridge_a", "event", json!(null))
            .unwrap();
    ...
    #[test]
    fn test_emit_to_named_not_found() {
        let multi = MultiBridge::new("multi");
        multi.add(CallbackBridge::new("bridge_a", |_, _| Ok(())));

        let result = multi.emit_to_named("nonexistent", "event", json!(null));
        assert!(result.is_err());
    }

    #[test]
    fn test_emit_to_named_disconnected() {
        let multi = MultiBridge::new("multi");
        let bridge = CallbackBridge::new("bridge_a", |_, _| Ok(()));
        bridge.disconnect().unwrap();
        multi.add(bridge);

        let result = multi.emit_to_named("bridge_a", "event", json!(null));
        assert!(result.is_err());
    }
}
