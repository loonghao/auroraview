...
#[rstest]
fn run_args_assets_root_requires_html() {
...
#[rstest]
fn run_args_no_url_no_html_succeeds_parsing() {
...
#[rstest]
fn pack_args_url_frontend_conflict() {
    let result =
...
#[case(1920, 1080)]
#[case(0, 0)]
fn run_args_dimensions(#[case] w: u32, #[case] h: u32) {
    let args = RunArgs::try_parse_from([
        "run",
        "--url",
        "https://a.com",
...
#[case(800u32, 600u32)]
#[case(1440, 900)]
fn pack_args_dimensions(#[case] w: u32, #[case] h: u32) {
...
fn run_args_title_empty_string() {
    let args = RunArgs::try_parse_from(["run", "--url", "https://a.com", "--title", ""]).unwrap();
...
fn run_args_height_zero() {
    let args = RunArgs::try_parse_from(["run", "--url", "https://a.com", "--height", "0"]).unwrap();
...
fn pack_args_title_long_string() {
    ...
    let args = PackArgs::try_parse_from(["pack", "--title", title]).unwrap();
...
fn run_args_url_and_title_pairs(#[case] url: &str, #[case] title: &str) {
    let args = RunArgs::try_parse_from(["run", "--url", url, "--title", title]).unwrap();
